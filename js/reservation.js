document.addEventListener('DOMContentLoaded', () => {
    ...
    // 2. Логіка кліку по кнопці "Оплатити тур"
    ...
    if (btnSubmit) {
        btnSubmit.addEventListener('click', (e) => {
            ...
            // Якщо все заповнено — створюємо модалку динамічно
            if (isValid) {
    ...
    // 3. ДЕЛЕГУВАННЯ КЛІКУ: Слухаємо клік по кнопці "На головну" на рівні всього документа
    document.addEventListener('click', (e) => {
        if (e.target && e.target.id === 'btn-dynamic-home') {
            e.preventDefault();
            console.log("Клік по кнопці 'На головну' успішно перехоплено!");
            localStorage.clear(); 
            
            // Чистий відносний шлях без крапки попереду, щоб Vite правильно розрулив підпапку GitHub
